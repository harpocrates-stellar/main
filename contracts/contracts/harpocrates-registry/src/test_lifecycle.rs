...
///   - `get_proof_history` paginates correctly (offset + limit).
...
///   - Abuse limits: MAX_HISTORY_ENTRIES_PER_PROOF saturation and
///     MAX_HISTORY_LIMIT query cap are enforced.
...
fn lifecycle_anonymous_emits_history() {
    let (env, contract_id, _) = init_registry();
    let client = HarpocratesRegistryClient::new(&env, &contract_id);
    ...
    let history = client.get_proof_history(&proof_id, 0, 10);
    ...
    assert_ne!(env.events().all(), [].as_slice(), "expected history event after registration");
}

#[test]
fn lifecycle_anonymous_verified_emits_history() {
    let (env, contract_id, admin, _) = init_with_verifier();
    let client = HarpocratesRegistryClient::new(&env, &contract_id);
...
fn lifecycle_revoke_emits_history() {
    ...
    assert_ne!(env.events().all(), [].as_slice(), "expected history event after revocation");
}
...
fn lifecycle_verify_emits_history() {
    ...
    assert_ne!(env.events().all(), [].as_slice(), "expected history event after verification");
}
...
fn lifecycle_expire_emits_history_and_updates_status() {
    ...
    assert_ne!(env.events().all(), [].as_slice(), "expected history event after expiry");
}
...
fn lifecycle_correct_emits_history_and_updates_metadata() {
    ...
    assert_ne!(env.events().all(), [].as_slice(), "expected history event after correction");
}
...
fn lifecycle_history_pagination() {
    ...
    let page1 = client.get_proof_history(&proof_id, 0, 2);
    assert_eq!(page1.len(), 2);
    assert_eq!(page1.get(0).unwrap().action, ProofLifecycleAction::Registered as u32);
    assert_eq!(page1.get(1).unwrap().action, ProofLifecycleAction::Revoked as u32);

    let page2 = client.get_proof_history(&proof_id, 2, 2);
    assert_eq!(page2.len(), 1);
    assert_eq!(page2.get(0).unwrap().action, ProofLifecycleAction::Verified as u32);

    let page3 = client.get_proof_history(&proof_id, 3, 10);
...
fn lifecycle_history_limit_exceeded() {
    let (env, contract_id, _) = init_registry();
    let client = HarpocratesRegistryClient::new(&env, &contract_id);
    let source = Address::generate(&env);
    let proof_id = b32(&env, 0x91);

    client.register_source(&source, &b32(&env, 0x92), &b32(&env, 0x93), &proof_id);
    client.get_proof_history(&proof_id, 0, MAX_HISTORY_LIMIT + 1);
}
...
fn lifecycle_history_saturation_rejects_new_entries() {
    ...
    env.storage()
        .persistent()
        .set(&seq_key, &MAX_HISTORY_ENTRIES_PER_PROOF);
...
fn lifecycle_unknown_proof_has_empty_history() {
    ...
    assert_eq!(client.get_proof_history(&unknown, 0, 10).len(), 0);
}
