...
impl HarpocratesRegistry {
    ...
    pub fn register_anonymous_verified(
            ...
            || input_len == SILENT_WITNESS_V3_INPUT_LEN
        {
            ...
            verify_anonymous_proof(&env, public_inputs, proof);
    ...
    pub fn register_lineage(
        ...
        let record = LineageRecord {
            parent_proof_ids: lineage_parents_to_bytes(&env, &parent_proof_ids),
...
/// Serialize a parent proof-id list into a storage `Bytes` blob:
/// concatenated 32-byte identifiers (length is always a multiple of 32).
fn lineage_parents_to_bytes(env: &Env, id_list: &Vec<BytesN<32>>) -> Bytes {
    let mut buf = Bytes::new(env);
    for id in id_list.iter() {
        let mut arr = [0u8; 32];
        id.copy_into_slice(&mut arr);
...
fn verify_external_proof_checked(
    env: &Env,
    verifier: &Address,
    public_inputs: Bytes,
    proof: Bytes,
...
fn verify_external_proof(env: &Env, verifier: &Address, public_inputs: Bytes, proof: Bytes) {
    if !verify_external_proof_checked(env, verifier, public_inputs, proof) {
        panic_with_error!(env, RegistryError::InvalidProof);
    }
}

/// Verify a proof against the current verifier, honoring the rotation overlap
/// window: while `current_ledger <= activation_ledger + overlap_window` proofs
/// accepted by the previous verifier remain valid. This keeps registrations
/// made with the old verifier's certificates working during the hand-over.
fn verify_anonymous_proof(env: &Env, public_inputs: Bytes, proof: Bytes) {
    let active = get_active_verifier(env);
    let state = get_verifier_rotation_state(env);
    let in_overlap = state.activation_ledger > 0
        && u64::from(env.ledger().sequence())
            <= state.activation_ledger.saturating_add(state.overlap_window);

    if in_overlap {
        if let Some(previous) = state.previous_verifier {
            if verify_external_proof_checked(env, &previous, public_inputs.clone(), proof.clone()) {
                return;
            }
        }
    }
    verify_external_proof(env, &active, public_inputs, proof);
}
...
///
/// NOTE: This struct is intentionally `Clone`-only; `BytesN<32>` is not
/// `Copy` on the pinned SDK, so array init uses `core::array::from_fn`.
#[derive(Clone)]
...
fn parse_aggregated_public_inputs(
    ...
    let mut elements = core::array::from_fn(|_| default_element.clone());
...
fn supersession_reverse_key(env: &Env, superseding_proof_id: &BytesN<32>) -> BytesN<32> {
    ...
    superseding_proof_id.copy_into_slice(&mut superseding);
    pre_image[12..].copy_from_slice(&superseding);
...
fn dispatch_timelocked_action(env: &Env, proposal: &TimelockProposal) {
    match proposal.action {
        ...
        3 => {
            ...
            let ttl_secs = u64::from_be_bytes(payload_bytes[0..8].try_into().unwrap());
            env.storage()
                .persistent()
                .set(&DataKey::ProofTtl, &ttl_secs);
        }
