...
impl MockScopedVerifier {
    pub fn verify_proof(_env: Env, public_inputs: Bytes, proof: Bytes) {
        // Accept the v1 (160-byte) and v2 (224-byte) frames, plus the
        // pre-domain legacy lengths.
...
/// Build a v1 (160-byte, domain-tagged) silent witness public-input blob.
#[cfg(test)]
fn v1_public_inputs(
    env: &Env,
...
/// Build a v2 (224-byte, domain-tagged) scoped silent witness public-input blob.
#[cfg(test)]
fn v2_public_inputs(
    env: &Env,
    video_hash: &BytesN<32>,
    ...
    let mut dg = [0u8; 32];
    expected_domain_tag(env).copy_into_slice(&mut dg);
    buf[192..224].copy_from_slice(&dg);
    Bytes::from_array(env, &buf)
}
