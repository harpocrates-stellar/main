...
fn reporter_cooldown_enforced() {
    let (env, cid, _admin, _source, _issuer, spid, _) = make_env();
    // A non-zero clock so the cooldown record is distinguishable from
    // "never opened" and `resolved_at` reads back non-zero.
...
fn resolve_dispute_happy_path() {
    ...
    // Non-zero clock so resolve_dispute stores a non-zero resolved_at.
    env.ledger().set_timestamp(1_700_000_000);
    let client = HarpocratesRegistryClient::new(&env, &cid);
    let reporter = Address::generate(&env);
